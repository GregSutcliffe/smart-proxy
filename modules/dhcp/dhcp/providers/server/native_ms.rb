...
module Proxy::DHCP
  ...
  class NativeMS < Server
    ...
    def loadSubnets
      ...
      execute(cmd, msg).each do |line|
        ...
        if match = line.match(/^\s*([\d\.]+)\s*-\s*([\d\.]+)\s*-\s*(Active|Disabled)/)
          next if (managed_subnets = Proxy::DhcpPlugin.settings.dhcp_subnets) and !managed_subnets.include? "#{match[1]}/#{match[2]}"
    ...
    def execute cmd, msg=nil, error_only=false
      tsecs = 5
      response = nil
      interpreter = Proxy::DhcpPlugin.settings.x86_64 ? 'c:\windows\sysnative\cmd.exe' : 'c:\windows\system32\cmd.exe'
